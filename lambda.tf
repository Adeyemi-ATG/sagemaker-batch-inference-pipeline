...
resource "aws_lambda_function" "pipeline_notification_lambda" {
  filename         = "${path.module}/lambda_function_payload.zip"  # The file should be in the root directory
  function_name    = var.notification_function_name
  role             = aws_iam_role.iam_for_lambda.arn
  handler          = "index.lambda_handler"
  source_code_hash = filebase64sha256("${path.module}/lambda_function_payload.zip")  # Correct path to the file
  runtime          = "python3.9"
